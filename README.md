...
Recording now offers selectable Hollywood and private Direct FFmpeg hardware backends with 720p/1080p/1440p, 30/60 FPS, bitrate, rate-control, tuning, H.264 profile/level, keyframe, and AAC settings. The right-side `Live Stream` tab implements a bounded single-encode proof of concept for Twitch, YouTube, Kick, and custom RTMP/RTMPS endpoints. Audio/file/network work has been moved off real-time callbacks, and stream keys remain memory-only and unlogged. This code is host-tested and ARM64-built only until an explicitly authorized Quest pass validates direct video, final MP4s, service connections, A/V sync, and performance. Companion output, TV casting, chat, Discord video, optional FBT, SpringBones, terrain feet, and additional avatar formats are not implemented.
