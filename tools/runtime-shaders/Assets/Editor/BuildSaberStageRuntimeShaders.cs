...
// File responsibility:
// - Builds the Android runtime shader AssetBundle from the Unity editor project.
...
internal static class BuildSaberStageRuntimeShaders
{
    private const string OutputVariable = "SABERSTAGE_RUNTIME_SHADER_OUTPUT";
    ...
    public static void BuildAndroid()
    {
        ...
        var build = new AssetBundleBuild {
            assetBundleName = "saberstage_runtime_shaders",
            assetNames = new[] {
        ...
        if (manifest == null || !File.Exists(Path.Combine(output, "saberstage_runtime_shaders")))
            throw new InvalidOperationException("Unity did not create the SaberStage Android runtime shader bundle.");
        ...
        var bundle = AssetBundle.LoadFromFile(Path.Combine(output, "saberstage_runtime_shaders"));
