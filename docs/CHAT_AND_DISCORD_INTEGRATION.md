...
| Feed SaberStage video and selected stream audio into the local Discord client's stream/screen share | Implemented through the separate `TCP Media Receiver` Android activity and Android 14's user-selected single-app sharing flow; this is app-window sharing with capturable app audio, not a Discord SDK video-source API |
...
give SaberStage control of Discord. Instead, the separately installed receiver
...
over authenticated loopback. The receiver hardware-decodes video and publishes
...
SaberStage opens Discord first and the receiver's source activity second. This
...
the receiver is visible may replace the receiver's root task. Android package
visibility is not treated as authoritative because an installed sideloaded
receiver can be hidden from `PackageManager`. Only an explicit receiver launch that
returns `ActivityNotFoundException` is reported as a missing installation.

After the authenticated protocol handshake, the receiver reports the exact AVC
...
receiver window prevents it from appearing in Discord's captured picture.
...
for the compatibility gates that must be proven before replacing the receiver.
