...
Humanoid binding measures arm and leg segments, approximate arm span, shoulder and hip width, every available hips-to-head segment, feet/toes, rest bend directions, eye position, head-to-eye pose, and skeletal floor. The eye pose uses VRM's actual first-person bone plus offset through the constructed Unity hierarchy when available, then mapped eye bones/fallback geometry. Neutral tracking calibration stores HMD and hand poses, floor/tracking origin, forward, standing HMD height, and controller-to-wrist fallbacks. The separate versioned player profile adds player arm span/confidence, multi-pose grip/reach, lean/step signatures, crouch/duck tendencies, and turn timing without modifying VRM geometry. Uniform scale is selected from player-to-avatar arm span when both measurements are credible; a bounded `standing HMD height / avatar skeletal eye height` fallback remains for incomplete or low-confidence data. Neutral head placement is decoupled from the absolute HMD position and follows relative HMD movement. Optional bounded vertical leg/torso retargeting can then match player height without changing arm length or shoulder width.

The Quest HMD drives the avatar eye/view anchor, and the desired Head bone pose is solved backward through the calibrated head-to-eye transform. In both menus and gameplay, SaberStage now prefers the active visible Beat Saber `Saber::_handleTransform`; menu discovery accepts a visible handle hierarchy even when the menu has disabled its `Saber` behavior, rejects retained handles farther than 45 cm from the live controller, and selects the closest per-side candidate. If the menu exposes no usable Saber component, the wrist remains controller-driven but the fingers still close around the visible controller-attached grip. A tracked saber handle bypasses the controller-only offset and is a hard end-effector invariant: clavicle assistance and up to ten-percent emergency visual stretch absorb proportion mismatch, but the final hand stays on the known handle instead of visibly releasing the saber. The pre-anchor anatomical reach error remains diagnostic. Controller-only targets retain the ordinary five-percent stretch cap and unreachable-target clamp.

A source-specific grip-to-hand-bone quaternion is calibrated against the anatomical rest chain. Per-side grip and reach fits are used only above explicit confidence thresholds; otherwise the neutral controller-to-wrist target and measured avatar arm length remain authoritative. The tracked saber remains a hard positional target. Its hand rotation receives a wider 105-degree anatomical cone, while inferred controller/menu rotation retains the 70-degree cone; this preserves normal pronation and backhand cuts without allowing a transient target to turn the wrist inside-out. Humanoid finger bones are captured from each avatar's own rest hierarchy, their curl axes are derived from that model's palm/segment geometry, and conservative per-joint curls wrap the fingers around the live saber without assuming shared VRM local axes. Neither the eye nor tracked hand positions are smoothed by the body estimator.
...
For an ordinary reachable pelvis-to-head span, measured spine chords are laid along one solved circular arc. This preserves every segment length, both endpoints, and one continuous bend direction. Before that solve, asymmetric anatomical limits allow useful forward attack/lunge motion but strongly constrain backward C-bowing and excessive lateral rubber-body lean; excess displacement is transferred to pelvis translation and the existing step system. Degenerate or unreachable spans use a fixed-size, maximum-eight-pass FABRIK fallback guided toward a smooth curve; an end-anchored root shift is permitted only for a genuinely unreachable chain. Signed adjacent forward/lateral bends are measured and a sharp direction reversal raises the objective spine-Z diagnostic. Accumulated chain length distributes body yaw, residual head rotation, side/forward bend, and the bounded hand-midpoint contribution. The final eye and head rotation targets remain exact without overwriting the neck-to-head connection after the chain solve.

## Pelvis, lean, crouch, and translation

Horizontal HMD displacement is first measured relative to a persistent body translation. Motion inside a leg/spine/shoulder/eye-normalized anatomical limit is primarily lean: the spine bends while the pelvis follows only a small share and feet remain unchanged. The lateral torso lean has a hard believable maximum. Moderate sustained displacement begins moving the body/pelvis after a short dwell, while excess beyond the lean limit becomes translation immediately. Pelvis ground projection is independently clamped around the planted support anchors; the user-adjustable planted-leg boundary is reapplied after spine root correction so body translation cannot escape as an ankle-originated whole-body lean. Exceeding that boundary makes the support-step request urgent.

The geometric vertical estimator compares HMD height loss with forward displacement in the current torso frame. During saber gameplay, it maintains a slowly rising stance-height baseline that cannot ratchet downward during ducks. This lets a player calibrate upright but naturally play a few centimetres lower without being treated as permanently squatting. Mostly vertical loss lowers and slightly sets back the pelvis for a squat. Simultaneous forward displacement blends toward a hip hinge with less pelvis drop and more forward spine bend. Backward displacement cannot enter the forward-hinge term. Maximum drop and setback are normalized by measured leg reach, and pelvis height is capped against planted-leg reach so standing taller straightens the legs instead of putting the avatar on its toes. No fitted polynomial or inherited model constants are used.
...
Each foot stores planted/current/start/destination poses, state, reason, progress, and duration. A `PLANTED` foot retains its exact world pose. Ideal stance is recalculated from pelvis projection, scaled hip width, the user stance-width multiplier, torso yaw, and small capped translation/yaw prediction, but never directly moves an anchor.
...
`BodySolverTuning.hpp` contains every principal body threshold and rate under a named field. Most distance thresholds are fractions of measured leg reach, hip width, or calibrated eye height. Values are independently selected SaberStage starting points and must be tuned from Quest recordings; they are not copied from Basis, RenIK, Qavatars, or RootMotion Final IK. The runtime-facing torso lean, planted-leg lean, stance-width, and backward-spine controls scale these named limits rather than introducing alternate solver paths.

The Avatar panel's `Write Diagnostic Log` action logs, without per-frame spam: HMD/head/eye poses and eye error; pelvis, lateral lean, crouch, forward hinge, translation and predicted support margin; each spine segment direction, signed adjacent bends and reversal warning; each arm's source, shoulder/target, measured lengths, reach ratio plus source-local min/average/max, elbow flexion/pole, pre-anchor reach error, final hand error, hard-grip-anchor state, target/grip-offset/final wrist quaternions and wrist error; and each foot's state, request reason, destination, progress/duration, leg reach, and knee pole. It also includes solve time/count, Transform reads/writes, spine error/passes, and limb reachability.
...
Host tests cover calibration and explicit first-person eye anchors; smooth fixed-length spine solving; neutral, half-bent, near-chest, cross-body, near-full and unreachable arm targets; signed elbow-hemisphere sweeps; exact reachable grip position; hard tracked-saber positional anchoring with inverted-wrist rotation protection; bounded controller stretch, shoulder and wrist behavior; low-confidence grip/reach fallback; controller-derived versus saber-derived player fits and source transitions; personalized return lean, persistent slow/fast translation, diagonal lean, head-only tilt, controller-only motion, crouch/duck and turn behavior; HMD translation/yaw/pitch/roll with exact avatar-eye coincidence and neck length; spine-reversal rejection; independently reduced torso and planted-leg support boundaries; stance-width scaling; backward-only spine restriction; early translation/predicted-support stepping; planted anchors, one-foot-at-a-time steps, support-foot stability, step lift/landing, pivot steps, airborne entry/landing, deterministic reset/reseed, persistence fallback, and zero heap allocations. All six CTest targets and all twenty-five repository/tooling tests pass, and the Android ARM64 library compiles.
