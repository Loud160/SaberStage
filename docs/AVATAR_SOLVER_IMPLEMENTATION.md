...
The Quest HMD drives the avatar eye/view anchor, and the desired Head bone pose is solved backward through the calibrated head-to-eye transform. In menus, wrist targets remain controller poses composed with their explicit calibration offsets. In gameplay, the active Beat Saber `Saber::_handleTransform` bypasses the controller-only offset and supplies the visible grip pose. A tracked saber handle is a hard end-effector invariant: clavicle assistance and up to ten-percent emergency visual stretch absorb proportion mismatch, but the final hand stays on the known handle instead of visibly releasing the saber. The pre-anchor anatomical reach error remains diagnostic. Controller-only/menu targets retain the ordinary five-percent stretch cap and unreachable-target clamp.

A source-specific grip-to-hand-bone quaternion is calibrated against the anatomical rest chain. Per-side grip and reach fits are used only above explicit confidence thresholds; otherwise the neutral controller-to-wrist target and measured avatar arm length remain authoritative. Gameplay saber rotation is not clamped away from its tracked grip, while inferred controller/menu rotation retains the 70-degree wrist cone. Humanoid finger bones are captured from each avatar's own rest hierarchy, their curl axes are derived from that model's palm/segment geometry, and conservative per-joint curls wrap the fingers around the live saber without assuming shared VRM local axes. Neither the eye nor tracked hand targets are smoothed by the body estimator.
