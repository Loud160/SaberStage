...
The optional movable preview is a separate framed native BSML floating screen styled after Big Screen's performance panel. It has an upper `SaberStage | Primary` section, a lower `Grab anywhere to move` section, no visible native grab handle, and an invisible grab surface covering the full panel. Its world position, rotation, scale, selected camera, and visible state use versioned settings persistence. Movement is saved after the screen remains stable for 0.5 seconds. `Reset Preview` places it at a safe position in front of the current HMD while preserving its visibility choice.

Preview graphics use Unity UI layer 5 so the HMD can display them. The docked preview and persistent popout use the same native BSML `RawImage`, alpha-independent material, and spectator-texture assignment path. The previous popout-only `MeshRenderer` quad diverged from the working docked implementation and remained blank on Quest, so it is no longer used. The working docked/floor preview keeps its existing spectator exclusion, and the movable popout now shares it: its screen root is hidden through the `CanvasGroup` alpha path and its `RawImage` is snapshot-culled during spectator rendering, so recordings never contain the panel or a recursive feed. The popout also uses its own opaque material instance instead of sharing the docked monitor's material. The previews do not change the HMD transform and do not create an encoder. Floating-screen construction is deferred until the active scene has left `GameLoader` and BSML's main-menu dependency container exists; merely deferring from `late_load` to the first runtime update is not sufficient. All screen/image allocations are checked before use.
