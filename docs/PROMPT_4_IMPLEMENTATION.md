...
Preview graphics use Unity UI layer 5 so the HMD can display them. Immediately around the spectator camera's manual render, only the preview hierarchies' generated `CanvasRenderer` meshes are culled and then restored synchronously. This keeps the previews out of their own camera feed without deactivating, fading, or changing the layer of a retained world-space Canvas. The popout's `RawImage` is constructed directly under that Canvas using the same proven pattern as Big Screen's native image panels, rather than carrying parsed-layout state into a standalone floating screen. The previews do not change the HMD transform and do not create an encoder. Floating-screen construction is deferred until the active scene has left `GameLoader` and BSML's main-menu dependency container exists; merely deferring from `late_load` to the first runtime update is not sufficient. All screen/image allocations are checked before use.
