...
Preview graphics use Unity UI layer 5 so the HMD can display them. The docked preview and persistent popout use the same native BSML `RawImage`, alpha-independent material, and spectator-texture assignment path. The previous popout-only `MeshRenderer` quad diverged from the working docked implementation and remained blank on Quest, so it is no longer used. The working docked/floor preview keeps its existing spectator exclusion. The movable popout is not registered under a capture-exclusion `CanvasGroup` and its `RawImage` is not culled, allowing the camera to render the entire popout surface and deliberately produce recursive feedback for diagnosis and demonstration. The previews do not change the HMD transform and do not create an encoder. Floating-screen construction is deferred until the active scene has left `GameLoader` and BSML's main-menu dependency container exists; merely deferring from `late_load` to the first runtime update is not sufficient. All screen/image allocations are checked before use.
