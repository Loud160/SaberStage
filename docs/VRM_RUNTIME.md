...
`Load Avatar` is the normal complete lifecycle operation: it constructs and validates the selected file, binds its Animator to the trackerless solver, applies the active profile's saved avatar settings, and performs a final neutral-pose reset. `Unload Avatar` releases it, while `Visible` hides or shows an already loaded avatar without unloading it. `Bind Tracking` and `Reset Avatar Pose` remain recovery actions rather than required setup steps, and the expression buttons exercise morph binding. A missing, unreadable, or rejected file never prevents camera, preview, or recording startup. An uncalibrated player profile never exposes a generic visible avatar: calibration may stage the selected humanoid invisibly so its tracking capture can run, then enables it only after the profile is completed and saved.
