...
The compositor owns the single final image before encoding. Stage 1 contains only the selected game camera. Later `BroadcastScene` documents can reference typed, bounded sources such as the game camera, song metadata, score/combo/energy, static text/image, status, and optional viewer-facing chat.
