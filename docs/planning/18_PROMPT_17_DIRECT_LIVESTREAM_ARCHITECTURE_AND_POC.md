...
> Implementation checkpoint: the right panel now has a functional `Live Stream` tab for Twitch, YouTube, Kick, and custom RTMP/RTMPS endpoints. `DirectLivestreamSink` uses bounded asynchronous queues, the required state machine, reconnect backoff, visible queue/drop health, provider-specific staged server/key fields, and one Direct FFmpeg/MediaCodec H.264 encode. Go Live starts a stream-only capture with no local media files unless the user had already explicitly started a compatible local recording, in which case the stream shares that existing encode. `Use Once` remains memory-only; `Save in Settings` is an explicit local plaintext persistence choice, with password masking and support-log redaction. Host tests and the ARM64 build are required before deployment; 720p30/1080p30 service broadcasts, A/V sync, reconnect, certificate verification, memory, and gameplay cost still require explicit on-headset validation. Android Keystore protection is not claimed.
