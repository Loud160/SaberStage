...
The preview is an HMD-only consumer with its own semantic placement, scale, visibility, camera ID, and reset-to-visible action. It does not appear in broadcast culling masks. Hiding the panel releases its demand; if capture also is idle, no spectator render is scheduled. Grabbing the camera may temporarily enlarge the preview, matching the useful Camera2 workflow without copying implementation.
