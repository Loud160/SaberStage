...
| [VRM Qavatars](https://github.com/BSQ-VRM/VRM-Qavatars), loader-only inspection at public commit `466ea5beefc8da205a31008f98913387c8be927c` | Historical evidence that GLB/VRM data can be converted to Quest Unity objects; public Unity API feasibility for image decode, skinned meshes, and humanoid Avatar construction | Architectural/API feasibility only. SaberStage's parser, neutral model, validation, coordinate conversion, runtime ownership, and material policy are independently authored. Qavatars `customTypes/FinalIK` was not opened or used, and no Qavatars loader source was copied or translated. |
| [VRM 0.x specification](https://github.com/vrm-c/vrm-specification/blob/master/specification/0.0/README.md) | Normative VRM 0.x JSON structures, required humanoid mapping, right-handed Y-up coordinates, `-Z` model-forward rule, MToon metadata, blend-shape, first-person, and secondary-animation contracts | File-format implementation reference only |
| [ozz-animation](https://github.com/guillaumeblanc/ozz-animation), revision `744eb9d99f606eda849acb0b1204f7a3dc20bca1` | MIT two-bone IK target softening, reachability, pole/twist inputs, and analytic joint construction | Narrow adaptation in `src/avatar/TwoBoneIK.cpp`, marked `MIT_OZZ_DERIVED`; complete notice retained in source and `docs/THIRD_PARTY_NOTICES.md`; SIMD/matrix framework not imported |
...
| RapidJSON (VRM parser) | Reused existing dependency for bounded GLB JSON decoding | MIT; no new binary dependency. Binary accessors and VRM 0.x validation are independently implemented in SaberStage. |
